...
namespace Grid3D
{
    ...
    // converts a position in space to which grid index cell it is in
    CUDA_CALLABLE_MEMBER inline GridIndex positionToGridIndex(VectorMath::vec3 p, double dx)
    {
        double inv_dx = 1.0 / dx;
    ...
    // converts a grid index to a position
    CUDA_CALLABLE_MEMBER inline void GridIndexToPosition(GridIndex g, double dx,
                                                         double *x, double *y, double *z)
    {
        *x = (double)g.i * dx;
        *y = (double)g.j * dx;
    ...
    // converts a grid index to a position
    CUDA_CALLABLE_MEMBER inline VectorMath::vec3 GridIndexToPosition(int i, int j, int k, double dx)
    {
