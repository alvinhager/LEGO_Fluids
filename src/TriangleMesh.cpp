...
void TriangleMesh::writeMeshToASCIIPLY(std::string filename)
{
    {
        ...
        const int triangleNum = (int)triangles.size();
        ...
        {
            VectorMath::vec3 vertex = vertices[vi];
        ...
        {
            const Triangle &triangle = triangles[ti];
            outFile << "3 ";

            for (int vi = 0; vi < 3; ++vi)
                outFile << triangle.tri_vertex_indices[vi] << " ";
...
bool TriangleMesh::loadASCIIPLY(std::string PLYFilename)
{
    std::ifstream myfile(PLYFilename);
    std::string myString;

    if (!myfile.is_open())
    {
        std::cout << "Error opening PLY file";
        return 0;
    }
    ...
    while (i < numVertices)
    {
        ...
        float v1 = std::stof(out[0]);
        float v2 = std::stof(out[1]);
        float v3 = std::stof(out[2]);

        vertices.push_back(VectorMath::vec3(v1, v2, v3));
        i++;
    }
    ...
    while (i < numFaces)
    {
        ...
        int t1 = std::stoi(out[1]);
        int t2 = std::stoi(out[2]);
        int t3 = std::stoi(out[3]);
...
bool TriangleMesh::loadBOBJ(std::string BOBJFilename)
{
    ...
    std::vector<VectorMath::vec3> vertices(numverts);
    if (numverts > 0)
    {
...
{
    std::vector<VectorMath::vec3> temp_vertices;
    std::vector<VectorMath::vec3> temp_normals;
...
void TriangleMesh::writeMeshToPLY(std::string filename)
{
    ...
    if (isColorEnabled)
    {
        binsize = headersize + 3 * (sizeof(float) * (int)vertices.size() + sizeof(unsigned char) * (int)vertices.size()) + (sizeof(unsigned char) + 3 * sizeof(int)) * (int)triangles.size();
    }
    else
    {
        binsize = headersize + 3 * sizeof(float) * (int)vertices.size() + (sizeof(unsigned char) + 3 * sizeof(int)) * (int)triangles.size();
    }
    ...
    {
        t = triangles[i];
        verts[0] = t.tri_vertex_indices[0];
        verts[1] = t.tri_vertex_indices[1];
        verts[2] = t.tri_vertex_indices[2];
...
void TriangleMesh::writeMeshToOBJ(std::string filename)
{
    ...
    {
        t = triangles[i];
        v1 = t.tri_vertex_indices[0] + 1;
        v2 = t.tri_vertex_indices[1] + 1;
        v3 = t.tri_vertex_indices[2] + 1;

        str << "f " << v1 << "//" << v1 << " " << v2 << "//" << v2 << " " << v3 << "//" << v3 << std::endl;
    }
...
void TriangleMesh::translate(VectorMath::vec3 t)
{
...
bool TriangleMesh::_loadPLYVertexData(std::ifstream *file, std::string &header)
{
    ...
    if (isColorEnabled)
    {
        ...
        for (int i = 0; i < numVertices; i++)
        {
            ...
            vertexcolors.push_back(VectorMath::vec3(r / 255.0f, g / 255.0f, b / 255.0f));
        }
    }
    else
    {
        vertices.assign((VectorMath::vec3 *)vertexData, (VectorMath::vec3 *)vertexData + numVertices);
    }
    delete[] vertexData;
...
bool TriangleMesh::_loadPLYVertexDataASCII(std::ifstream *file, std::string &header)
{
    int numVertices;
    bool success = _getNumVerticesInPLYHeader(header, &numVertices);
    if (!success)
    ...
    {
        std::cout << "Successfully read vertex Data" << std::endl;
    }
...
bool TriangleMesh::_loadPLYTriangleData(std::ifstream *file, std::string &header)
{
    ...
    for (int i = 0; i < numFaces; i++)
    {
        ...
        memcpy(&(t.tri_vertex_indices), faceData + offset, 3 * sizeof(int));
        offset += 3 * sizeof(int);

        if (t.tri_vertex_indices[0] < 0 || t.tri_vertex_indices[0] >= numVertices ||
            t.tri_vertex_indices[1] < 0 || t.tri_vertex_indices[1] >= numVertices ||
            t.tri_vertex_indices[2] < 0 || t.tri_vertex_indices[2] >= numVertices)
        {
            return false;
        }
