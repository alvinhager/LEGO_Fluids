...
// default constructor
CUDA_CALLABLE_MEMBER VectorMath::vec3::vec3() : x(0.0), y(0.0), z(0.0)
{
}

// parameter constructor
CUDA_CALLABLE_MEMBER VectorMath::vec3::vec3(float x, float y, float z) : x(x), y(y), z(z)
{
}

// default destructor
CUDA_CALLABLE_MEMBER VectorMath::vec3::~vec3()
{
}
...
CUDA_CALLABLE_MEMBER VectorMath::vec3 VectorMath::operator-(const VectorMath::vec3 &v1, const VectorMath::vec3 &v2)
{
    return VectorMath::vec3(v1.x - v2.x, v1.y - v2.y, v1.z - v2.z);
}
...
CUDA_CALLABLE_MEMBER float VectorMath::vec3::operator[](int i)
{
    //CUSTOM_ASSERT(i >= 0 && i <= 2);
    return (&x)[i];
}
...
// return one cross section of matrix
CUDA_CALLABLE_MEMBER VectorMath::vec3 VectorMath::mat3::operator[](int i)
{
    //CUSTOM_ASSERT(i <= 2 && i >= 0);
    return vec3(m[3 * i], m[3 * i + 1], m[3 * i + 2]);
}
