...
MeshLevelSet::MeshLevelSet(int isize, int jsize, int ksize, double dx) : _isize(isize), _jsize(jsize), _ksize(ksize), _dx(dx), _closestTriangles(isize + 1, jsize + 1, ksize + 1, -1)
{
...
{
    CUSTOM_ASSERT(_phi.isIndexInRange(i, j, k));
    return _phi(i, j, k);
}

float MeshLevelSet::get(GridIndex g)
{
    CUSTOM_ASSERT(_phi.isIndexInRange(g));
    return _phi(g);
}

int MeshLevelSet::getClosestTriangleIndex(int i, int j, int k)
{
    CUSTOM_ASSERT(_closestTriangles.isIndexInRange(i, j, k));
    return _closestTriangles(i, j, k);
}

int MeshLevelSet::getClosestTriangleIndex(GridIndex g)
{
    CUSTOM_ASSERT(_closestTriangles.isIndexInRange(g));
...
float MeshLevelSet::trilinearInterpolate(VectorMath::vec3 pos)
{
    return (float)Interpolation::trilinearInterpolate(pos, _dx, _phi);
}

VectorMath::vec3 MeshLevelSet::trilinearInterpolateGradient(VectorMath::vec3 pos)
{
    VectorMath::vec3 grad;
...
float MeshLevelSet::getFaceWeightU(int i, int j, int k)
{
    ...
    return LevelsetFunctions::fractionInside(_phi(i, j, k),
                                             _phi(i, j + 1, k),
                                             _phi(i, j, k + 1),
                                             _phi(i, j + 1, k + 1));
}
...
void MeshLevelSet::calculateUnion(MeshLevelSet &levelset)
{
    ...
    CUSTOM_ASSERT(oi == _isize && oj == _jsize && ok == _ksize);
    ...
    {
        t = omesh->triangles[i];
        t.tri_vertex_indices[0] += indexOffset;
        t.tri_vertex_indices[1] += indexOffset;
        t.tri_vertex_indices[2] += indexOffset;
                                                  ...
                                                  Array3D<int> &intersectionCounts)
{
    ...
    for (size_t tidx = 0; tidx < _mesh.triangles.size(); tidx++)
    {
        ...
        VectorMath::vec3 p = _mesh.vertices[t.tri_vertex_indices[0]];
        VectorMath::vec3 q = _mesh.vertices[t.tri_vertex_indices[1]];
        VectorMath::vec3 r = _mesh.vertices[t.tri_vertex_indices[2]];
    ...
    while (startidx < (int)queue.size())
    {
        ...
        for (int nidx = 0; nidx < 6; nidx++)
        {
            ...
            if (Grid3D::isGridIndexInRange(n, isize, jsize, ksize) && _closestTriangles(n) != -1)
            {
                ...
                double dist = _pointToTriangleDistance(gpos, _mesh.vertices[t.tri_vertex_indices[0]],
                                                       _mesh.vertices[t.tri_vertex_indices[1]],
                                                       _mesh.vertices[t.tri_vertex_indices[2]]);
                if (dist < _phi(g))
                {
...
float MeshLevelSet::_pointToTriangleDistance(VectorMath::vec3 x0, VectorMath::vec3 x1,
                                             VectorMath::vec3 x2,
                                             VectorMath::vec3 x3)
{
    // first find barycentric coordinates of closest point on infinite plane
    VectorMath::vec3 x13 = x1 - x3;
    VectorMath::vec3 x23 = x2 - x3;
    VectorMath::vec3 x03 = x0 - x3;

    float m13 = VectorMath::lengthsq(x13);
    float m23 = VectorMath::lengthsq(x23);
    float d = VectorMath::dot(x13, x23);
    float invdet = 1.0f / fmax(m13 * m23 - d * d, 1e-30f);
    float a = VectorMath::dot(x13, x03);
    float b = VectorMath::dot(x23, x03);
    ...
    { // if we're inside the triangle
        return VectorMath::length(x0 - (w23 * x1 + w31 * x2 + w12 * x3));
    }
    else
    {
...
float MeshLevelSet::_pointToSegmentDistance(VectorMath::vec3 x0, VectorMath::vec3 x1, VectorMath::vec3 x2)
{
    VectorMath::vec3 dx = x2 - x1;
    double m2 = VectorMath::lengthsq(dx);
    // find parameter value of closest point on segment
    float s12 = (float)(VectorMath::dot(x2 - x0, dx) / m2);
    if (s12 < 0)
    {
        s12 = 0;
    }
    ...
    return VectorMath::length(x0 - (s12 * x1 + (1 - s12) * x2));
}
